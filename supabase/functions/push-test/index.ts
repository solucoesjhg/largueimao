...
serve(async (req: Request) => {
  ...
  try {
    const { platform, token, title, body, data } = await req.json();

    if (!platform || !token || !title || !body) {
      return new Response(JSON.stringify({ error: "Missing required fields (platform, token, title, body)" }), {
        status: 400,
    ...
    if (platform === "ios" || platform === "apns") {
      ...
      return new Response(JSON.stringify({ error: "Invalid platform. Use 'ios' or 'android'." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }

    const result = await provider.send({ token, title, body, data });
